...
type OpenSwapCovenantExclusion = {
...
function openSwapPrintable(value: any): string | null {
...
function openSwapReferenceEntry(reference: any): any | null {
  return reference && typeof reference === "object" && reference.entry ? reference.entry : null;
}

function openSwapCovenantIdFromReference(reference: any): string | null {
  const entry = openSwapReferenceEntry(reference);
  const canonical = entry ? entry.covenantId : undefined;
  return canonical === null || canonical === undefined ? null : openSwapPrintable(canonical);
}

function openSwapOutpointKeyFromValue(outpoint: any): string {
  if (!outpoint) return "";
  if (typeof outpoint === "string") return outpoint;
  if (outpoint && typeof outpoint.toJSON === "function") return openSwapOutpointKeyFromValue(outpoint.toJSON());
  const transactionId = openSwapPrintable(outpoint.transactionId ?? outpoint.transaction_id ?? outpoint.txid);
...
function openSwapOutpointKeyFromReference(reference: any): string {
  const entry = openSwapReferenceEntry(reference);
  const candidates = [
  ...
  for (const candidate of candidates) {
    const key = openSwapOutpointKeyFromValue(candidate);
    if (key) return key;
  }
  return "";
}

function applyOpenSwapCovenantExclusion(args: {
  networkId: RpcNetworkId;
  address: string;
  entries: any[];
  inspectionKind: string;
}): { entries: any[]; exclusion: OpenSwapCovenantExclusion } {
  const rawEntries = Array.isArray(args.entries) ? args.entries : [];
  const covenantOutpoints = new Set<string>();

  for (const entry of rawEntries) {
    const covenantId = openSwapCovenantIdFromReference(entry);
    const key = covenantId ? openSwapOutpointKeyFromReference(entry) : "";
    if (key) covenantOutpoints.add(key);
  }

  const spendableEntries = rawEntries.filter((entry) => {
    const key = openSwapOutpointKeyFromReference(entry);
  ...
  app.post("/api/open-swaps/offer", async (req, res) => {
    ...
    try {
      ...
      if (isPrepare) {
        ...
        const ownerAddress = String(active.address0 || "").trim();
        const ownerUtxos = await rpc.getUtxosByAddresses({ addresses: [ownerAddress] });
        ...
        const openSwapCommitCovenantExclusion = applyOpenSwapCovenantExclusion({
          networkId,
          address: ownerAddress,
          entries: ownerEntries,
          inspectionKind: "open_swap_offer_commit_covenant_exclusion_v1"
        });
        if (!openSwapCommitCovenantExclusion.entries.length) {
          return res.status(409).json({
            ok: false,
            reason: "open_swap_offer_commit_only_covenant_utxos",
            stage,
            covenant_exclusion: openSwapCommitCovenantExclusion.exclusion
          });
        }

        const commitFunding = await selectOpenSwapCommitFundingEntries({
          ownerEntries: openSwapCommitCovenantExclusion.entries,
          commitAmountSompi,
          listP2shAddress,
