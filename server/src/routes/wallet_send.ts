...
export async function handleWalletSend(req: Request, res: Response, ctx: WalletSendCtx): Promise<any> {
  ...
  try {
    ...
    if (stage === "krc_commit_build") {
      ...
      const krc20CommitCovenantExclusion = await applyNormalSendCovenantExclusion({
        networkId,
        address: active.address0,
        entries
      });
      const krc20CommitEntries = krc20CommitCovenantExclusion.entries;

      if (!krc20CommitEntries.length) {
        return res.json({
          ok: false,
          reason: "krc20_commit_only_covenant_utxos",
          error: "Only covenant-bearing UTXOs are available. KRC20 commit funding is blocked for those outputs.",
          covenant_exclusion: krc20CommitCovenantExclusion.exclusion
        });
      }

      const safeEntries = krc20CommitEntries.map((e: any) => ({
        outpoint:
