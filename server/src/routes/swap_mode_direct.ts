...
type DirectSwapCovenantExclusion = {
...
function directSwapPrintable(value: any): string | null {
...
function directSwapReferenceEntry(reference: any): any | null {
  return reference && typeof reference === "object" && reference.entry ? reference.entry : null;
}

function directSwapCovenantIdFromReference(reference: any): string | null {
  const entry = directSwapReferenceEntry(reference);
  const canonical = entry ? entry.covenantId : undefined;
  return canonical === null || canonical === undefined ? null : directSwapPrintable(canonical);
}

function directSwapOutpointKeyFromValue(outpoint: any): string {
  if (!outpoint) return "";
  if (typeof outpoint === "string") return outpoint;
  if (outpoint && typeof outpoint.toJSON === "function") return directSwapOutpointKeyFromValue(outpoint.toJSON());
  const transactionId = directSwapPrintable(outpoint.transactionId ?? outpoint.transaction_id ?? outpoint.txid);
...
function directSwapOutpointKeyFromReference(reference: any): string {
  const entry = directSwapReferenceEntry(reference);
  const candidates = [
  ...
  for (const candidate of candidates) {
    const key = directSwapOutpointKeyFromValue(candidate);
    if (key) return key;
  }
  return "";
}

function applyDirectSwapCovenantExclusion(args: {
  networkId: RpcNetworkId;
  address: string;
  entries: any[];
  inspectionKind: string;
}): { entries: any[]; exclusion: DirectSwapCovenantExclusion } {
  const rawEntries = Array.isArray(args.entries) ? args.entries : [];
  const covenantOutpoints = new Set<string>();

  for (const entry of rawEntries) {
    const covenantId = directSwapCovenantIdFromReference(entry);
    const key = covenantId ? directSwapOutpointKeyFromReference(entry) : "";
    if (key) covenantOutpoints.add(key);
  }

  const spendableEntries = rawEntries.filter((entry) => {
    const key = directSwapOutpointKeyFromReference(entry);
...
export function registerSwapModeDirectRoutes(app: Express, ctx: SwapModeDirectCtx): void {
  ...
  app.post("/api/swaps/offer", async (req, res) => {
    ...
    try {
      ...
      const ownerEntries = ownerUtxos && Array.isArray((ownerUtxos as any).entries) ? ((ownerUtxos as any).entries as any[]) : [];
      if (!ownerEntries.length) {
        return res.status(409).json({ ok: false, reason: "no_utxos" });
      }
      const makerCommitCovenantExclusion = applyDirectSwapCovenantExclusion({
        networkId,
        address: active.address0,
        entries: ownerEntries,
        inspectionKind: "direct_swap_offer_commit_covenant_exclusion_v1"
      });
      if (!makerCommitCovenantExclusion.entries.length) {
        return res.status(409).json({
          ok: false,
          reason: "direct_swap_offer_commit_only_covenant_utxos",
          covenant_exclusion: makerCommitCovenantExclusion.exclusion,
          signing_enabled: false,
          broadcasting_enabled: false,
          minting_enabled: false
        });
    ...
    // rid is defined earlier based on request stage (prepare generates, other stages use offerRid)
      ...
      if (isPrepare) {
        ...
        const commitEntries: any[] = makerCommitCovenantExclusion.entries;
        ...
        try {
          ...
          if (msg.includes("Storage mass exceeds maximum")) {
            return res.status(409).json({
              ...
              stage,
              utxoCount: makerCommitCovenantExclusion.entries.length,
              excludedCovenantUtxoCount: makerCommitCovenantExclusion.exclusion.excluded_entries
            });
          }
          throw e;
        }
      ...
      if (isComplianceWallet) {
        ...
        const makerAnchorCovenantExclusion = applyDirectSwapCovenantExclusion({
          networkId,
          address: active.address0,
          entries: makerEntries,
          inspectionKind: "direct_swap_maker_anchor_covenant_exclusion_v1"
        });
        if (!makerAnchorCovenantExclusion.entries.length) {
          return res.status(409).json({
            ok: false,
            reason: "direct_swap_maker_anchor_only_covenant_utxos",
            stage,
            covenant_exclusion: makerAnchorCovenantExclusion.exclusion,
            signing_enabled: false,
            broadcasting_enabled: false,
            minting_enabled: false
          });
        }
        const makerAnchorEntries = makerAnchorCovenantExclusion.entries;
        ...
        if (cachedAnchor && typeof cachedAnchor.transactionId === "string") {
          ...
          anchorEntry = makerAnchorEntries.find((e: any) => {
          ...
          const sorted = makerAnchorEntries.slice().sort((a: any, b: any) => {
  ...
  app.post("/api/swaps/accept", async (req, res) => {
    ...
    try {
      ...
      const takerCovenantExclusion = applyDirectSwapCovenantExclusion({
        networkId,
        address: active.address0,
        entries: takerEntries,
        inspectionKind: "direct_swap_accept_taker_covenant_exclusion_v1"
      });
      if (!takerCovenantExclusion.entries.length) {
        return res.status(409).json({
          ok: false,
          reason: "direct_swap_accept_taker_only_covenant_utxos",
      ...
      takerEntries = takerCovenantExclusion.entries
        .filter((e: any) => {
          const op = e && e.outpoint ? e.outpoint : null;
