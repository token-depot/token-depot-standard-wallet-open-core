  ...
  function signKcc20AtomicOpenTakerClaimBuild(build, priv0Hex) {
    return kaspaReadyOrThrow().then(function (kaspa) {
      ...
      var fundingInputIndexes = Array.isArray(ctx.native_kas_funding_input_indexes)
        ? ctx.native_kas_funding_input_indexes.map(function (n) { return Number(n); })
        : [];
      var fundingInputOutpoints = Array.isArray(ctx.native_kas_funding_input_outpoints)
        ? ctx.native_kas_funding_input_outpoints.map(function (v) { return String(v || '').trim(); })
        : [];
      ...
      if (!fundingInputIndexes.length) throw new Error('kcc20_open_taker_claim_funding_input_indexes_missing');
      if (fundingInputIndexes.some(function (index) { return !Number.isInteger(index) || index < 0 || !signSet.has(index); })) throw new Error('kcc20_open_taker_claim_funding_input_not_signable');
      if (fundingInputOutpoints.length !== fundingInputIndexes.length) throw new Error('kcc20_open_taker_claim_funding_outpoint_count_mismatch');
      if (nativeFundingInputs.length !== fundingInputIndexes.length) throw new Error('kcc20_open_taker_claim_native_funding_input_count_mismatch');
      fundingInputIndexes.forEach(function (fundingInputIndex, i) {
        var funding = nativeFundingInputs[i] || {};
        if (Number(funding.input_index) !== fundingInputIndex) throw new Error('kcc20_open_taker_claim_native_funding_input_index_mismatch');
        if (String(funding.outpoint || '').trim() !== fundingInputOutpoints[i]) throw new Error('kcc20_open_taker_claim_native_funding_outpoint_mismatch');
      });
      if (signIndexes.length !== 1 + fundingInputIndexes.length) throw new Error('kcc20_open_taker_claim_unexpected_sign_indexes');
      ...
      return kcc20OpenSha256HexText(output1SpkBytesHex).then(function (output1SpkTextHash) {
        ...
        return kcc20OpenSha256HexText(signedSafeJson).then(function (sha) {
          return {
            ...
            native_kas_funding_input_indexes: fundingInputIndexes,
            native_kas_funding_input_count: fundingInputIndexes.length,
  ...
  function init() {
    ...
    function renderGroupedOpenOfferTable(target, groups, emptyText, options) {
      ...
      groups.forEach(function (group) {
        ...
        var kcc20OpenCovenantId = kcc20OpenSell ? normalizeOfferCa(kcc20OpenSell.asset_covenant_id) : '';
        if (kcc20OpenSell && !/^[0-9a-f]{64}$/i.test(kcc20OpenCovenantId) && /^[0-9a-f]{64}$/i.test(normalizedSellCa)) {
          kcc20OpenCovenantId = normalizedSellCa;
        }
        var sellSymbolLooksCa = !kcc20OpenDraft && /^[0-9a-f]{64}$/i.test(normalizedSellCa);
        ...
        if (kcc20OpenCovenantId) {
          var covenantIdLine = document.createElement('p');
          covenantIdLine.className = 'offer-sub';
          covenantIdLine.textContent = 'Covenant ID: ' + kcc20OpenCovenantId;
          details.appendChild(covenantIdLine);
        } else if (sellSymbolLooksCa) {
