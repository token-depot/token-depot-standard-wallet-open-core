...
(function () {
  ...
  var pendingKcc20AtomicDirectBuy = null;
  ...
  function signKcc20AtomicDirectTakerClaim(build, priv0Hex) {
    return kaspaReadyOrThrow().then(function (kaspa) {
      ...
      var ctx = build.signing_context_public && typeof build.signing_context_public === 'object' ? build.signing_context_public : {};
      var holderInputIndex = Number(ctx.holder_input_index);
      var fundingInputIndexes = Array.isArray(ctx.native_kas_funding_input_indexes)
        ? ctx.native_kas_funding_input_indexes.map(function (value) { return Number(value); })
        : [];
      var fundingInputOutpoints = Array.isArray(ctx.native_kas_funding_input_outpoints)
        ? ctx.native_kas_funding_input_outpoints.map(function (value) { return String(value || '').trim(); })
        : [];
      var nativeFundingInputs = Array.isArray(build.native_kas_funding_inputs)
      ...
      if (!fundingInputIndexes.length) {
        throw new Error('kcc20_atomic_swap_taker_claim_native_funding_inputs_missing');
      }
      if (fundingInputIndexes.some(function (index) { return !Number.isInteger(index) || index < 0 || !signSet[index]; })) {
      ...
      if (fundingInputOutpoints.length !== fundingInputIndexes.length) {
        throw new Error('kcc20_atomic_swap_taker_claim_native_funding_outpoint_count_mismatch');
      }
      if (nativeFundingInputs.length !== fundingInputIndexes.length) {
        throw new Error('kcc20_atomic_swap_taker_claim_native_funding_input_count_mismatch');
      }
      fundingInputIndexes.forEach(function (fundingInputIndex, i) {
        var expectedOutpoint = fundingInputOutpoints[i];
        var actualOutpoint = String(nativeFundingInputs[i] && nativeFundingInputs[i].outpoint || '').trim();
        if (actualOutpoint !== expectedOutpoint) {
          throw new Error('kcc20_atomic_swap_taker_claim_native_funding_outpoint_mismatch');
        }
        if (Number(nativeFundingInputs[i] && nativeFundingInputs[i].input_index) !== fundingInputIndex) {
          throw new Error('kcc20_atomic_swap_taker_claim_native_funding_index_mismatch');
        }
      });
      ...
      return {
        ...
        native_funding_signature_scripts_all_present: fundingInputIndexes.every(function (index) {
          return typeof tx.inputs[index].signatureScript === 'string' && tx.inputs[index].signatureScript.length > 0;
        })
      };
    });
  }
  ...
  function clearPendingKcc20AtomicDirectBuy(restoreButton) {
    var pending = pendingKcc20AtomicDirectBuy;
    pendingKcc20AtomicDirectBuy = null;
    if (restoreButton && pending && pending.clickedButton) {
      pending.clickedButton.disabled = false;
      pending.clickedButton.textContent = pending.originalButtonText || 'Buy';
    }
  }
  ...
  function handleKcc20AtomicDirectClaimClick(offer, clickedButton) {
    ...
    clearPendingKcc20AtomicDirectBuy(true);

    var originalButtonText = clickedButton ? clickedButton.textContent : '';
    if (clickedButton) {
      clickedButton.disabled = true;
      clickedButton.textContent = 'Preparing…';
    }

    updateFillPanel({
      summary: 'Preparing KCC20 Atomic Direct Swap purchase…',
      status: 'Building the taker purchase transaction from the selected live maker-lock.',
      psktText: '',
      sendText: '',
      canConfirm: false,
    ...
    buildKcc20AtomicDirectTakerClaim(offer)
      .then(function (build) {
        isSigningFill = false;
        pendingKcc20AtomicDirectBuy = {
          offer: offer,
          build: build,
          clickedButton: clickedButton || null,
          originalButtonText: originalButtonText || 'Buy'
        };
        updateFillPanel({
          summary: 'Buy KCC20 Atomic Direct Swap.',
          status: 'Build ready. Review the purchase, then click Confirm & Buy KCC20 Direct Swap.',
          psktText: JSON.stringify(atomicClaimPublicSummary(build, null, null), null, 2),
          sendText: '',
          canConfirm: true,
          buttonLabel: 'Confirm & Buy KCC20 Direct Swap'
        });
        if (clickedButton) clickedButton.textContent = 'Buy Pending';
      })
      .catch(function (err) {
        console.error('offers.atomicBuy build error', err);
        isSigningFill = false;
        clearPendingKcc20AtomicDirectBuy(true);
        ...
        if (reason === 'kcc20_atomic_swap_taker_claim_policy_requires_final_true_v3') {
          statusText = 'This is an older v1/v2 atomic swap offer. Use the newest Direct Swap offer, or refresh after hiding old/filled rows.';
        } else if (detail && detail !== reason) {
          statusText = reason + '\n' + detail;
        }
        updateFillPanel({
          summary: 'KCC20 Atomic Direct Swap purchase build failed.',
          status: statusText,
  ...
  function confirmKcc20AtomicDirectBuy() {
    var pending = pendingKcc20AtomicDirectBuy;
    ...
    if (!priv0Hex) {
      updateFillPanel({
        summary: 'Keyfile unlock required to buy this atomic swap.',
        status: 'Unlock your keyfile in the Wallet tab first, then click Confirm & Buy again.',
        psktText: JSON.stringify(atomicClaimPublicSummary(pending.build, null, null), null, 2),
        sendText: '',
        canConfirm: true,
        buttonLabel: 'Confirm & Buy KCC20 Direct Swap'
      });
    ...
    var signed = null;
    updateFillPanel({
      summary: 'Signing KCC20 Atomic Direct Swap purchase…',
      status: 'Signing the claim selector and native KAS funding input locally.',
      psktText: JSON.stringify(atomicClaimPublicSummary(pending.build, null, null), null, 2),
      sendText: '',
      canConfirm: false,
      buttonLabel: 'Signing…'
    ...
    signKcc20AtomicDirectTakerClaim(pending.build, priv0Hex)
      .then(function (signedOut) {
        signed = signedOut;
        updateFillPanel({
          summary: 'Submitting KCC20 Atomic Direct Swap purchase…',
          status: 'Signed locally. Submitting the purchase to testnet-10.',
          psktText: JSON.stringify(atomicClaimPublicSummary(pending.build, signedOut, null), null, 2),
          sendText: '',
          canConfirm: false,
          buttonLabel: 'Submitting…'
        });
        return submitKcc20AtomicDirectTakerClaim(pending.build, signedOut);
      })
      .then(function (submitOut) {
        isSigningFill = false;
        var section = $('fillSection');
        if (section) section.__reloadAfterClose = true;
        var summaryObj = atomicClaimPublicSummary(pending.build, signed, submitOut);
        var clickedButton = pending.clickedButton;
        pendingKcc20AtomicDirectBuy = null;
        updateFillPanel({
          summary: 'Success — KCC20 Atomic Direct Swap filled.',
          status: 'Success — KCC20 Atomic Direct Swap filled. Txid: ' + (summaryObj.submitted_txid || 'unknown'),
          statusClass: 'fill-success',
          psktText: JSON.stringify(summaryObj, null, 2),
          sendText: '',
          canConfirm: false,
          buttonLabel: 'Swap Filled',
          closeLabel: 'Close & Refresh Offers'
        });
        if (clickedButton) {
          clickedButton.disabled = true;
          clickedButton.textContent = 'Swap Filled';
        }
      })
      .catch(function (err) {
        console.error('offers.atomicBuy error', err);
        isSigningFill = false;
        var reason = err && (err.reason || err.message) ? String(err.reason || err.message) : String(err);
        var response = err && err.response ? err.response : null;
        var detail = response && response.error ? String(response.error) : '';
        var statusText = detail && detail !== reason ? (reason + '\n' + detail) : reason;
        clearPendingKcc20AtomicDirectBuy(true);
        updateFillPanel({
          summary: 'KCC20 Atomic Direct Swap purchase failed.',
          status: statusText,
          psktText: response ? JSON.stringify(response, null, 2) : '',
          sendText: '',
          canConfirm: false,
...
function handleConfirmFill() {
    ...
    if (pendingKcc20AtomicDirectBuy && pendingKcc20AtomicDirectBuy.build) {
      confirmKcc20AtomicDirectBuy();
      return;
    }
