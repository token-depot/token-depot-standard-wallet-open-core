  ...
  async function signKcc20AtomicDirectMakerLock(build, priv0Hex){
    ...
    const holderInputs = Array.isArray(signCtx.holder_inputs) ? signCtx.holder_inputs.map(function(input){
      return {
        ...
        holderScriptProfile: String(input.holder_script_profile || 'owner_signature_v1').trim() || 'owner_signature_v1'
      };
    }) : [];
    ...
    holderInputs.forEach(function(input){
      ...
      if (['owner_signature_v1', 'regulated_registry_control_v1'].indexOf(input.holderScriptProfile) === -1) throw new Error('kcc20_atomic_swap_maker_lock_holder_script_profile_unsupported');
    });

    const fundingInputIndexes = Array.isArray(signCtx.native_kas_funding_input_indexes)
      ? signCtx.native_kas_funding_input_indexes.map(function(v){ return Number(v); })
      : [];
    const fundingInputOutpoints = Array.isArray(signCtx.native_kas_funding_input_outpoints)
      ? signCtx.native_kas_funding_input_outpoints.map(function(v){ return String(v || '').trim(); })
      : [];
    const nativeFundingInputs = Array.isArray(build.native_kas_funding_inputs)
      ? build.native_kas_funding_inputs
      : [];
    if (fundingInputIndexes.some(function(index){ return !Number.isInteger(index) || index < 0; })) throw new Error('kcc20_atomic_swap_maker_lock_native_funding_input_index_invalid');
    if (fundingInputOutpoints.length !== fundingInputIndexes.length) throw new Error('kcc20_atomic_swap_maker_lock_native_funding_outpoint_count_mismatch');
    if (nativeFundingInputs.length !== fundingInputIndexes.length) throw new Error('kcc20_atomic_swap_maker_lock_native_funding_input_count_mismatch');
    fundingInputIndexes.forEach(function(fundingInputIndex, i){
      if (!signSet.has(fundingInputIndex)) throw new Error('kcc20_atomic_swap_maker_lock_native_funding_input_not_signable');
      const expectedOutpoint = fundingInputOutpoints[i];
      const buildOutpoint = String(nativeFundingInputs[i] && nativeFundingInputs[i].outpoint || '').trim();
      if (!expectedOutpoint || buildOutpoint !== expectedOutpoint) throw new Error('kcc20_atomic_swap_maker_lock_native_funding_outpoint_mismatch');
    });
    if (signInputIndexes.length !== holderInputs.length + fundingInputIndexes.length) throw new Error('kcc20_atomic_swap_maker_lock_unexpected_sign_indexes');
    ...
    const encodeHolderSignatureScript = function(item, signature){
      if (item.input.holderScriptProfile !== 'regulated_registry_control_v1') {
        return item.script.encodePayToScriptHashSignatureScript(signature);
      }
      const signatureScriptHex = typeof signature === 'string'
        ? signature
        : new kaspaSdk.ScriptBuilder({ flags: { covenantsEnabled: true } }).addData(signature).drain();
      const branchAndRedeemScriptHex = new kaspaSdk.ScriptBuilder({ flags: { covenantsEnabled: true } })
        .addOp(kaspaSdk.Opcodes.OpTrue)
        .addData(item.input.redeemScriptHex)
        .drain();
      return signatureScriptHex + branchAndRedeemScriptHex;
    };

    scripts.forEach(function(item){
      fillKcc20DirectSignatureScript(tx, item.input.inputIndex, encodeHolderSignatureScript(item, dummySig), 'kcc20_atomic_swap_maker_lock_holder_input_missing_for_dummy_sig');
    });
    scripts.forEach(function(item){
      const sig = kaspaSdk.createInputSignature(tx, item.input.inputIndex, priv0, null);
      fillKcc20DirectSignatureScript(tx, item.input.inputIndex, encodeHolderSignatureScript(item, sig), 'kcc20_atomic_swap_maker_lock_holder_input_missing_for_final_sig');
    });
    fundingInputIndexes.forEach(function(fundingInputIndex){
    ...
    return {
      ...
      native_funding_input_used: fundingInputIndexes.length > 0,
      native_funding_input_indexes: fundingInputIndexes,
      native_funding_input_count: fundingInputIndexes.length,
      ...
      native_funding_signature_scripts_present: fundingInputIndexes.map(function(index){ return typeof tx.inputs[index].signatureScript === 'string' && tx.inputs[index].signatureScript.length > 0; }),
      native_funding_signature_scripts_all_present: fundingInputIndexes.every(function(index){ return typeof tx.inputs[index].signatureScript === 'string' && tx.inputs[index].signatureScript.length > 0; }),
  ...
  async function buildAndSignKcc20DirectHolderTransfer(payload, planOut, priv0Hex){
    ...
    const holderInputs = Array.isArray(signCtx.holder_inputs) ? signCtx.holder_inputs.map(function(input){
      return {
        ...
        sourceHolderOutpoint: String(input.source_holder_outpoint || ''),
        redeemScriptHex: String(input.source_holder_redeem_script_hex || '').trim(),
        holderScriptProfile: String(input.holder_script_profile || 'owner_signature_v1').trim()
      };
    }) : [];
    const fundingInputIndexes = Array.isArray(signCtx.native_kas_funding_input_indexes)
      ? signCtx.native_kas_funding_input_indexes.map(function(v){ return Number(v); })
    ...
    const fundingInputOutpoints = Array.isArray(signCtx.native_kas_funding_input_outpoints)
      ? signCtx.native_kas_funding_input_outpoints.map(function(v){ return String(v || '').trim(); })
      : [];
    const nativeFundingInputs = build && build.transfer_plan && Array.isArray(build.transfer_plan.native_kas_funding_inputs)
      ? build.transfer_plan.native_kas_funding_inputs
      : [];
    ...
    holderInputs.forEach(function(input){
      ...
      if (['owner_signature_v1', 'regulated_registry_control_v1'].indexOf(input.holderScriptProfile) === -1) throw new Error('kcc20_direct_swap_holder_script_profile_unsupported');
    });
    if (fundingInputIndexes.some(function(index){ return !Number.isInteger(index) || index < 0; })) throw new Error('kcc20_direct_swap_native_funding_input_index_invalid');
    if (fundingInputOutpoints.length !== fundingInputIndexes.length) throw new Error('kcc20_direct_swap_native_funding_outpoint_count_mismatch');
    if (nativeFundingInputs.length !== fundingInputIndexes.length) throw new Error('kcc20_direct_swap_native_funding_input_count_mismatch');
    ...
    fundingInputIndexes.forEach(function(fundingInputIndex, i){
      ...
      if (!signSet.has(fundingInputIndex)) throw new Error('kcc20_direct_swap_native_funding_input_not_signable');
      if (holderInputs.some(function(input){ return input.inputIndex === fundingInputIndex; })) throw new Error('kcc20_direct_swap_native_funding_overlaps_holder_input');
      if (duplicateFundingIndexes.has(fundingInputIndex)) throw new Error('kcc20_direct_swap_duplicate_native_funding_input_index');
      duplicateFundingIndexes.add(fundingInputIndex);
      if (String(fundingInputOutpoints[i] || '').trim() !== fundingOutpoint) throw new Error('kcc20_direct_swap_native_funding_outpoint_mismatch');
      if (Number(fundingInput.input_index) !== fundingInputIndex) throw new Error('kcc20_direct_swap_native_funding_input_plan_mismatch');
    });
    if (signInputIndexes.length !== holderInputs.length + fundingInputIndexes.length) throw new Error('kcc20_direct_swap_unexpected_sign_indexes');
    ...
    scripts.forEach(function(item){
      fillKcc20DirectSignatureScript(tx, item.input.inputIndex, encodeHolderSignatureScript(item, dummySig), 'kcc20_direct_swap_holder_input_missing_for_dummy_sig');
    });
    scripts.forEach(function(item){
      const sig = kaspaSdk.createInputSignature(tx, item.input.inputIndex, priv0, null);
      fillKcc20DirectSignatureScript(tx, item.input.inputIndex, encodeHolderSignatureScript(item, sig), 'kcc20_direct_swap_holder_input_missing_for_final_sig');
    });
    fundingInputIndexes.forEach(function(fundingInputIndex){
      const fundingSig = kaspaSdk.createInputSignature(tx, fundingInputIndex, priv0, null);
    ...
    if (isKcc20DirectSwapPayload(payload)) {
      ...
      if (isKcc20AtomicOpen) {
        try {
          ...
          kcc20OpenOfferQuantity = requestedQuantity;
        } catch (e) {
      ...
      const cfm = await window.CW_showConfirm({
        ...
        network,
        confirmLabel: isKcc20AtomicOpen
          ? (kcc20OpenOfferQuantity > 1 ? ('Sign & Create ' + kcc20OpenOfferQuantity + ' Atomic Open Swaps') : 'Sign & Create Atomic Open Swap')
          : 'Sign & Create Atomic Direct Swap',
        cancelLabel: 'Cancel',
        sendingText: isKcc20AtomicOpen
          ? (kcc20OpenOfferQuantity > 1 ? ('Building Open atomic maker-lock 1 of ' + kcc20OpenOfferQuantity + '…') : 'Building Open atomic maker-lock…')
          : 'Building atomic maker-lock…'
      });
      ...
      let createdKcc20OpenCount = 0;
      try {
        const kcc20OfferCount = isKcc20AtomicOpen ? kcc20OpenOfferQuantity : 1;
        let lastBuild = null;
        let lastSubmitOut = null;

        for (let kcc20OfferIndex = 0; kcc20OfferIndex < kcc20OfferCount; kcc20OfferIndex++) {
          const sequenceText = isKcc20AtomicOpen && kcc20OfferCount > 1
            ? (' ' + (kcc20OfferIndex + 1) + ' of ' + kcc20OfferCount)
            : '';

          try { cfm.setSendingText && cfm.setSendingText(isKcc20AtomicOpen ? ('Building Open atomic maker-lock' + sequenceText + '…') : 'Building atomic maker-lock…'); } catch (_) {}
          if (r) r.textContent = isKcc20AtomicOpen ? ('Building Open atomic maker-lock' + sequenceText + '…') : 'Building atomic maker-lock…';
          ...
          try { cfm.setSendingText && cfm.setSendingText(isKcc20AtomicOpen ? ('Signing Open atomic maker-lock' + sequenceText + '…') : 'Signing atomic maker-lock…'); } catch (_) {}
          if (r) r.textContent = isKcc20AtomicOpen ? ('Signing Open atomic maker-lock' + sequenceText + '…') : 'Signing atomic maker-lock…';
          ...
          try { cfm.setSendingText && cfm.setSendingText(isKcc20AtomicOpen ? ('Submitting Open atomic maker-lock' + sequenceText + '…') : 'Submitting atomic maker-lock…'); } catch (_) {}
          if (r) r.textContent = isKcc20AtomicOpen ? ('Submitting Open atomic maker-lock' + sequenceText + '…') : 'Submitting atomic maker-lock…';
          ...
          if (isKcc20AtomicOpen) createdKcc20OpenCount += 1;
          ...
          lastBuild = build;
          lastSubmitOut = submitOut;
        }

        if (r) r.innerHTML = isKcc20AtomicOpen
          ? renderKcc20AtomicOpenMakerLockPanel(lastBuild, lastSubmitOut)
          : renderKcc20AtomicDirectMakerLockPanel(lastBuild, lastSubmitOut);
        const submittedMsg = isKcc20AtomicOpen
          ? (kcc20OpenOfferQuantity > 1
              ? (kcc20OpenOfferQuantity + ' KCC20 Atomic Open Swaps created. Each maker-lock is live and listed in Open Swap Offers.')
        ...
        const partial = isKcc20AtomicOpen && createdKcc20OpenCount > 0
          ? ('Created ' + createdKcc20OpenCount + ' of ' + kcc20OpenOfferQuantity + ' Open swaps before failure. ')
          : '';
        if (r) r.textContent = 'Make Offer failed: ' + partial + msg;
        try { cfm.setError('Make Offer failed: ' + partial + msg); } catch (_) {}
        return;
      }
    }
