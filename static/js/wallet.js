...
(function(){
  ...
  const OMA_L1_REGULATED_PROFILE_KIND = 'KCC20-Regulated';
  const OMA_L1_REGULATED_CONTROL_FLAGS_MAX = 0x3f;

  function omaL1RegulatedProfileOrNull(tok) {
    const hasKind = !!tok && Object.prototype.hasOwnProperty.call(tok, 'regulated_profile_kind');
    const hasProfile = !!tok && Object.prototype.hasOwnProperty.call(tok, 'regulated_profile');
    if (!hasKind && !hasProfile) return null;

    if (!hasKind || !hasProfile || tok.regulated_profile_kind !== OMA_L1_REGULATED_PROFILE_KIND) {
      throw new Error('oma_l1_regulated_profile_invalid');
    }

    const profile = tok.regulated_profile;
    ...
    if (
      ...
      rv !== 1 ||
      !/^[0-9a-f]{64}$/.test(pr) ||
      !Number.isInteger(pe) || pe < 1 || pe > 0xffffffff ||
      !Number.isInteger(fc) || fc < 1 || fc > OMA_L1_REGULATED_CONTROL_FLAGS_MAX
    ) {
      throw new Error('oma_l1_regulated_profile_invalid');
    }
  ...
  function omaL1RegulatedProfileKey(profile) {
    return profile ? `${profile.rg}:${profile.rv}:${profile.pr}:${profile.fc}` : 'standard';
  }
  ...
  function omaL1AggregateTrackedTokensForSelect(h) {
    ...
    for (const tok of omaL1TrackedTokensFromHoldings(h)) {
      ...
      const regulatedProfile = omaL1RegulatedProfileOrNull(tok);
      const regulatedProfileKey = omaL1RegulatedProfileKey(regulatedProfile);
      if (!byAsset.has(covenantId)) {
        byAsset.set(covenantId, {
          asset_covenant_id: covenantId,
          token_symbol: symbol,
          decimals,
          regulated_profile_kind: regulatedProfile ? OMA_L1_REGULATED_PROFILE_KIND : null,
          regulated_profile: regulatedProfile,
          regulated_profile_key: regulatedProfileKey,
          amount_raw_total: 0n,
          holder_lot_count: 0
        });
      }
      const row = byAsset.get(covenantId);
      if (row.regulated_profile_key !== regulatedProfileKey) {
        throw new Error('oma_l1_regulated_profile_mismatch');
      }
      if (regulatedProfile && row.regulated_profile && regulatedProfile.pe > row.regulated_profile.pe) {
        row.regulated_profile = { ...row.regulated_profile, pe: regulatedProfile.pe };
      }
  ...
  function renderTokenSelect(h) {
    ...
    for (const tok of omaL1Tokens) {
      ...
      const regulatedProfile = tok.regulated_profile || null;
      const opt = document.createElement('option');
      opt.value = omaL1SelectValue(tok);
      opt.textContent = regulatedProfile
        ? `${symbol} · ${OMA_L1_REGULATED_PROFILE_KIND} · ${shortCA(covenantId)}`
        : `${symbol} · OMA L1 · ${shortCA(covenantId)}`;
      opt.dataset.assetKind = 'oma_l1_covenant_token';
      opt.dataset.assetCovenantId = covenantId;
      opt.dataset.tokenSymbol = symbol;
      if (regulatedProfile) {
        opt.dataset.regulatedProfileKind = OMA_L1_REGULATED_PROFILE_KIND;
        opt.dataset.policyRegistryCovenantId = regulatedProfile.pr;
        opt.dataset.policyEpoch = String(regulatedProfile.pe);
        opt.dataset.controlFlags = String(regulatedProfile.fc);
      }
  ...
  function renderAssets(h) {
    ...
    for (const tok of omaTokens) {
      ...
      if (regulatedProfile) {
        tr.dataset.regulatedProfileKind = OMA_L1_REGULATED_PROFILE_KIND;
        tr.dataset.policyRegistryCovenantId = regulatedProfile.pr;
        tr.dataset.policyEpoch = String(regulatedProfile.pe);
        tr.dataset.controlFlags = String(regulatedProfile.fc);
      }

      const td1 = document.createElement('td');
      td1.appendChild(document.createTextNode(
        regulatedProfile
          ? `${symbol} · ${OMA_L1_REGULATED_PROFILE_KIND} · covenant `
          : `${symbol} · covenant `
      ));
      ...
      if (regulatedProfile) {
        ...
        profileLine.className = 'muted';
        const profileText = document.createElement('small');
        profileText.textContent = `Registry ${shortCA(regulatedProfile.pr)} · Epoch ${regulatedProfile.pe} · Controls ${regulatedProfile.fc}`;
        profileText.title = `Policy registry ${regulatedProfile.pr}; policy epoch ${regulatedProfile.pe}; control flags ${regulatedProfile.fc}; live holder profile verified by server`;
        profileLine.appendChild(profileText);
        td1.appendChild(profileLine);
      }
  ...
  function omaL1SignIndexesOrThrow(build, signCtx) {
    ...
    for (const input of holderInputs) {
      ...
      if (!['owner_signature_v1', 'regulated_registry_control_v1'].includes(input.holderScriptProfile)) {
        throw new Error('oma_l1_holder_transfer_script_profile_unsupported');
      }
    }
    ...
    const rawFundingInputIndexes = signCtx && Array.isArray(signCtx.native_kas_funding_input_indexes)
      ? signCtx.native_kas_funding_input_indexes
      : [];
    const fundingInputIndexes = rawFundingInputIndexes.map((value) => Number(value));
    const fundingInputOutpoints = signCtx && Array.isArray(signCtx.native_kas_funding_input_outpoints)
    ...
    if (fundingInputIndexes.some((value) => !Number.isInteger(value) || value < 0)) {
      throw new Error('oma_l1_holder_transfer_native_funding_input_index_invalid');
    }
    if (fundingInputOutpoints.length !== fundingInputIndexes.length) {
      throw new Error('oma_l1_holder_transfer_native_funding_outpoint_count_mismatch');
    }
    if (nativeFundingInputs.length !== fundingInputIndexes.length) {
      throw new Error('oma_l1_holder_transfer_native_funding_input_count_mismatch');
    }
    ...
    for (let i = 0; i < fundingInputIndexes.length; i += 1) {
      ...
      if (duplicateFundingIndexes.has(fundingInputIndex)) throw new Error('oma_l1_holder_transfer_duplicate_native_funding_input_index');
      duplicateFundingIndexes.add(fundingInputIndex);
      if (!expectedOutpoint || !fundingOutpoint || expectedOutpoint !== fundingOutpoint) throw new Error('oma_l1_holder_transfer_native_funding_outpoint_mismatch');
      if (Number(fundingInput.input_index) !== fundingInputIndex) throw new Error('oma_l1_holder_transfer_native_funding_input_plan_mismatch');
    }

    const expectedCount = holderInputs.length + fundingInputIndexes.length;
  ...
  function encodeOmaL1HolderSignatureScript(k, redeemScriptHex, signature, holderScriptProfile) {
    ...
    // createInputSignature() already returns an executable OP_DATA_65 signature script.
    // Preserve that script verbatim, then append the ordinary-branch selector and
    // the pushed redeem script. Uint8Array is used only for the mass placeholder
    // and must first be encoded as one data push.
  ...
  async function signOmaL1HolderTransferBuild(build, keyring) {
    ...
    const holderScripts = signPlan.holderInputs.map((input) => ({ input }));

    for (const item of holderScripts) {
      fillOmaL1SignatureScript(
        tx,
        item.input.inputIndex,
        encodeOmaL1HolderSignatureScript(k, item.input.redeemScriptHex, dummySig, item.input.holderScriptProfile),
        'oma_l1_holder_transfer_source_holder_input_missing'
      );
    }
    ...
    for (const item of holderScripts) {
      ...
      fillOmaL1SignatureScript(
        tx,
        item.input.inputIndex,
        encodeOmaL1HolderSignatureScript(k, item.input.redeemScriptHex, holderSignature, item.input.holderScriptProfile),
        'oma_l1_holder_transfer_source_holder_input_missing'
      );
    }

    for (const fundingInputIndex of signPlan.fundingInputIndexes) {
      const fundingSignature = k.createInputSignature(tx, fundingInputIndex, keyring.priv0, null);
      fillOmaL1SignatureScript(tx, fundingInputIndex, fundingSignature, 'oma_l1_holder_transfer_native_funding_input_missing');
    }
