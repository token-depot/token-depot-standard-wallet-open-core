(function () {
  ...
  const daaGuidanceState = {
    currentVirtualDaaScore: null,
    rate: null
  };
  ...
  function positiveIntegerText(value) {
    ...
    return /^[0-9]+$/.test(text) && BigInt(text) > 0n ? text : null;
  }

  function formatIntegerText(value) {
    const text = String(value == null ? "" : value).trim();
    if (!/^[0-9]+$/.test(text)) return text || "unavailable";
    return text.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  }

  function daaRateSummary(rate) {
    if (!rate || rate.basis !== "network_target") return "Network target DAA rate unavailable.";
    return `Network target: ≈${formatIntegerText(rate.score_per_second)} DAA/s · ${formatIntegerText(rate.score_per_minute)}/min · ${formatIntegerText(rate.score_per_hour)}/hour · ${formatIntegerText(rate.score_per_day)}/day.`;
  }

  function daaDurationEstimate(deltaText, rate) {
    const delta = positiveIntegerText(deltaText);
    const perSecond = rate ? positiveIntegerText(rate.score_per_second) : null;
    if (!delta || !perSecond) return "Estimated delay unavailable.";
    const deltaNumber = Number(delta);
    const perSecondNumber = Number(perSecond);
    if (!Number.isFinite(deltaNumber) || !Number.isFinite(perSecondNumber) || perSecondNumber <= 0) return "Estimated delay unavailable.";
    const seconds = deltaNumber / perSecondNumber;
    let value;
    let unit;
    if (seconds < 60) {
      value = seconds;
    ...
    return `Estimated delay: ≈${rounded} ${unit}${rounded === 1 ? "" : "s"} for ${formatIntegerText(delta)} DAA.`;
  }

  function renderDaaGuidance() {
    const rate = daaGuidanceState.rate;
    const rateText = daaRateSummary(rate);
    const scoreText = positiveIntegerText(daaGuidanceState.currentVirtualDaaScore) || "";
    const currentScoreField = $("ccEscrowCurrentDaaScore");
    if (currentScoreField) currentScoreField.value = scoreText;

    const guidanceRows = [
      ["ccAbsoluteTimeLockDaaDelta", "ccAbsoluteTimeLockDaaEstimate"],
      ["ccRelativeSequenceDaaDelta", "ccRelativeSequenceDaaEstimate"],
      ["ccC5LockDaaDelta", "ccC5LockDaaEstimate"],
      ["ccC6RecoveryDaaDelta", "ccC6RecoveryDaaEstimate"]
    ];
    guidanceRows.forEach(function (row) {
      const input = $(row[0]);
      const output = $(row[1]);
      if (output) setText(output, `${rateText} ${daaDurationEstimate(readField(input), rate)}`);
    });

    const escrowDelta = readField($("ccEscrowRefundDaaDelta"));
    const escrowEstimate = $("ccEscrowRefundDaaEstimate");
    if (escrowEstimate) setText(escrowEstimate, `${rateText} ${daaDurationEstimate(escrowDelta, rate)} Based on the latest Inspector refresh.`);
    const escrowTarget = $("ccEscrowRefundLockDaa");
    const delta = positiveIntegerText(escrowDelta);
    if (escrowTarget) {
      escrowTarget.value = scoreText && delta ? (BigInt(scoreText) + BigInt(delta)).toString() : "";
    }
  }

  function updateDaaGuidanceFromInspection(data) {
    const currentScore = positiveIntegerText(data && data.current_virtual_daa_score);
    const rate = data && data.daa_score_rate && data.daa_score_rate.basis === "network_target"
      ? data.daa_score_rate
      : null;
    if (currentScore) daaGuidanceState.currentVirtualDaaScore = currentScore;
    if (rate) daaGuidanceState.rate = rate;
    renderDaaGuidance();
  }

  function bindDaaGuidanceInputs() {
    [
      "ccAbsoluteTimeLockDaaDelta",
      "ccRelativeSequenceDaaDelta",
      "ccEscrowRefundDaaDelta",
      "ccC5LockDaaDelta",
      "ccC6RecoveryDaaDelta"
    ].forEach(function (id) {
      const input = $(id);
      if (input) input.addEventListener("input", renderDaaGuidance);
    });
    renderDaaGuidance();
  }
  ...
  function serverTrackedOmaL1ControllerRow(tok) {
    if (!tok || tok.asset_kind !== "oma_l1_covenant_token") return null;
    const controllerOnlyZeroSupply = tok.controller_only_zero_supply === true;
    const verificationStatus = String(tok.verification_status || "");
    if (
      verificationStatus !== "live_chain_recomputed_p2sh_match"
      && verificationStatus !== "live_chain_recomputed_controller_only_zero_supply_match"
    ) return null;
    ...
    return {
      ...
      covenant_id: covenantId,
      covenant_path: "server_tracked_record_live_verified",
      classification: controllerOnlyZeroSupply
        ? "verified_kcc20_regulated_zero_supply_controller"
        : "verified_oma_l1_controller_state",
      label: controllerOnlyZeroSupply
        ? `Verified ${symbol} KCC20-Regulated controller — zero supply`
        : `Verified ${symbol} controller`,
      registry_match: true,
      proof_status: verificationStatus,
      token: false,
      warning: controllerOnlyZeroSupply
        ? "Verified KCC20-Regulated zero-supply issuer controller. Use the regulated Issue workflow to create the first holder output. Controller carrier KAS remains blocked from normal KAS send."
        : "Verified OMA L1 controller state from server-tracked live-chain verification. Controller carrier KAS remains blocked from normal KAS send.",
      warning_class: controllerOnlyZeroSupply
        ? "verified_kcc20_regulated_zero_supply_controller_normal_send_blocked"
        : "verified_oma_l1_controller_state_normal_send_blocked",
      normal_send_eligible: false,
      verified_oma_l1_state: {
        proof_kind: controllerOnlyZeroSupply
          ? "oma_l1_server_tracked_controller_only_zero_supply_row_v1"
          : "oma_l1_server_tracked_controller_row_v1",
  ...
  function ensureInspectionPolishStyles() {
    ...
    style.textContent = `
      ...
      #ccInspectionPanel table[role="grid"] td {
        background: rgba(var(--td-skin-table-row-rgb), .94);
      ...
      #ccInspectionPanel table[role="grid"] tbody tr:nth-child(even) td {
        background: rgba(var(--td-skin-table-row-rgb), .88);
      }
      ...
      #ccInspectionPanel code {
        background: rgba(var(--td-skin-table-head-rgb), .72) !important;
      ...
      #ccInspectionPanel .cc-inspect-detail-row details {
        ...
        border-radius: 10px;
        padding: .45rem .65rem;
        background: rgba(var(--td-skin-panel-rgb), .72);
      }
      ...
      #ccInspectionPanel .cc-inspect-copy-field {
        ...
        border-radius: 10px;
        background: rgba(var(--td-skin-panel-rgb), .68);
        padding: .55rem .65rem;
        overflow-wrap: anywhere;
      }
      ...
      #ccInspectionPanel .cc-inspect-metadata-card {
        ...
        border-radius: 12px;
        padding: .7rem .8rem;
        background: rgba(var(--td-skin-panel-rgb), .74);
        overflow-wrap: anywhere;
      }
      #ccInspectionPanel .cc-inspect-metadata-card[open] {
        background: rgba(var(--td-skin-panel-rgb), .88);
      }
      ...
      #ccTemplateRegistryPanel details.cc-large-panel-collapse {
        border: 1px solid rgba(var(--td-skin-border-rgb), 0.34);
        border-radius: 14px;
        background:
          radial-gradient(circle at top left, rgba(var(--td-skin-primary-glow-rgb), 0.12), rgba(var(--td-skin-panel-rgb), 0.94));
        box-shadow:
          inset 0 0 0 1px rgba(var(--td-skin-white-rgb), 0.035),
          0 0 18px rgba(var(--td-skin-primary-glow-rgb), 0.12);
      }
      ...
      #ccTemplateRegistryPanel details.cc-large-panel-collapse > summary {
        ...
        gap: 1rem;
        padding: .85rem 1rem;
        color: var(--td-skin-text-strong, #f6fbff);
        font-weight: 800;
        letter-spacing: .02em;
      }
      ...
      #ccTemplateRegistryPanel .cc-large-panel-summary small {
        font-weight: 500;
        color: var(--td-skin-text-soft, rgba(255,255,255,.72));
        letter-spacing: 0;
      }
      #ccInspectionPanel .cc-large-panel-body,
      #ccTemplateRegistryPanel .cc-large-panel-body {
        border-top: 1px solid rgba(var(--td-skin-border-rgb), 0.18);
        background: rgba(var(--td-skin-panel-rgb), 0.58);
        padding: 1rem;
      }
    `;
  ...
  function inspectionSummaryHtml(data) {
    ...
    return `
      ...
      Current Virtual DAA Score: <strong>${escapeHtml(formatIntegerText(data.current_virtual_daa_score))}</strong><br>
      DAA rate: ${escapeHtml(daaRateSummary(data.daa_score_rate))}<br>
  ...
  function renderInspection(data, httpStatus) {
    ...
    updateDaaGuidanceFromInspection(data);
    ...
    setText(UI.inspectJson, JSON.stringify({
      ...
      current_virtual_daa_score: data.current_virtual_daa_score || null,
      daa_score_rate: data.daa_score_rate || null,
      node: data.node,
      counts: data.counts,
